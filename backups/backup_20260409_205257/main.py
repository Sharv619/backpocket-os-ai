...
async def handle_slash_command(message: str, conversation_id: str, context_data: dict) -> Optional[str]:
    """Handle slash commands from dashboard chat."""
    import re
    ...
    if cmd == "/pending":
        ...
        if not args:
            return "⚠️ Usage: `/approve <ref>` (e.g., `/approve 2026-04-00001`)"
        ref_id = args.strip()
        info = db.get_pending_approval(ref_id)
        if not info:
            return f"⚠️ Ref #{ref_id} not found."
        from services.gmail import send_email
        ...
        if result.get('status') == 'success':
            db.log_action(ref_id, 'approved', info.get('tier', ''), 'Dashboard slash command')
        ...
        if not match:
            return "⚠️ Usage: `/revise <ref> <feedback>` (e.g., `/revise 2026-04-00001 make it friendlier`)"
        ...
        email_obj = {"id": info['message_id'], "subject": info['subject'], "sender": info['sender']}
        loop = asyncio.get_running_loop()
        new_draft = await loop.run_in_executor(None, refine_draft, email_obj, original, feedback)
        info['draft_body'] = new_draft
        db.save_pending_approval(ref_id, info)
        return f"✏️ **Revised (Ref: {ref_id})**\n\n{new_draft[:500]}..."
    
    elif cmd == "/coach":
        ref_id = args.strip() if args else None
        if not ref_id:
            return "⚠️ Usage: `/coach <ref>` (e.g., `/coach 2026-04-00001`)"
        info = db.get_pending_approval(ref_id)
        if not info:
            return f"⚠️ Ref #{ref_id} not found."
...
Just type naturally — Twin understands everything!"""
        ...
        recent = f"{history[0]['action']} #{history[0]['ref_id']}" if history else "No recent actions"
...
async def twin_chat(request: Request):
    ...
    try:
        ...
        if message and message.startswith("/"):
            response = await handle_slash_command(message, conversation_id, context_data)
            if response:
                save_message_instant(conversation_id, "user", message)
                save_message_instant(conversation_id, "assistant", response)
                return {"response": response, "conversation_id": conversation_id, "is_command": True}
...
@app.get("/api/search-gmail")
async def api_search_gmail(q: str):
    """Search for emails across all Gmail accounts via web dashboard."""
    from services.gmail import search_emails, get_all_account_tokens
    loop = asyncio.get_running_loop()
...
async def api_search_emails(request: Request):
    """Semantic email search using natural language query."""
    try:
        data = await request.json()
...
# REMOVED: Duplicate /api/pending endpoint (original at line 272 returns {count, items} format)
# This old version returned {pending} — different shape. Dashboard uses the L272 version.
...
async def api_approve(request: ApproveRequest):
    ...
    try:
        ...
        hook_data = {"ref_id": ref_id, "draft_body": draft, "subject": subject, "sender": email_addr, "tier": info.get('tier', '')}
    ...
    if result['status'] == 'success':
        ...
        try:
            from services.hooks import run_hook
            hook_data = {"ref_id": ref_id, "action": "approved", "draft_body": draft, "subject": subject, "sender": email_addr, "tier": info.get('tier', '')}
...
async def api_create_hook(request: HookRequest):
    ...
    if request.event not in SUPPORTED_EVENTS:
        return {"status": "error", "message": f"Invalid event. Supported: {SUPPORTED_EVENTS}"}
    hook_id = save_hook(request.name, request.event, request.action_type, request.action_config, request.enabled)
...
@app.post("/api/chat/compact/{conversation_id}")
...
async def process_triaged_email(email, triage, loop):
    ...
    if triage.get('is_onboarding_triggered'):
        ...
        if not ai:
            ...
            return
        res = ai.models.generate_content(model='gemini-2.5-flash', contents=prompt, config={'response_mime_type': 'application/json'})
        import json
...
{raw_email_body}"""
        ...
        try:
            ...
            index_email(ref_id, email.get('subject', ''), snippet[:500], clean_email, datetime.now().strftime('%Y-%m-%d %H:%M'))
