...
def _get_client_whitelist() -> tuple:
    ...
    now = datetime.now()
    if _cache_last_loaded and (now - _cache_last_loaded).total_seconds() < _CACHE_TTL_SECONDS:
    ...
    try:
        ...
        logger.info(f"WHITELIST: Loaded {len(_client_whitelist_cache)} emails & {len(_domain_whitelist_cache)} domains.")
...
def _get_cached_priority_list():
    ...
    if not _cache_last_loaded or (datetime.now() - _cache_last_loaded).total_seconds() >= _CACHE_TTL_SECONDS:
...
def test_gemini_connection():
    ...
    try:
        ...
        response_text = response.text.strip() if response and response.text else "Connected"
...
def analyze_draft_with_coach(email_content, draft_response):
    ...
    """
    sys_prompt = "You are Cherry's Communication Coach (GPT-4o). Your job is to analyze her email drafts."
    prompt = f'''
...
'''
    import json
    res = get_openrouter_response(prompt, model="openai/gpt-4o", sys_prompt=sys_prompt, json_mode=True)
    if res:
        try:
...
def pre_triage_rules(email_content):
    ...
    """
    sender_email = str(email_content.get('clean_email') or email_content.get('sender') or '').lower().strip()
    ...
    if sender_email in priority_list:
        logger.info(f"🌟 DYNAMIC PRIORITY HIT (Email): {sender_email} — forcing Tier {priority_list[sender_email]}.")
        return {"tier": priority_list[sender_email], "reason": "Dynamic priority sender (Sheet)"}
    
    if sender_domain in priority_list:
        logger.info(f"🌟 DYNAMIC PRIORITY HIT (Domain): {sender_domain} — forcing Tier {priority_list[sender_domain]}.")
        return {"tier": priority_list[sender_domain], "reason": "Dynamic priority domain (Sheet)"}
    ...
    if "new user registered" in subject or "new client registered" in subject:
        ...
        return {
            "tier": 1, "reason": "New User Registered alert detected.",
            "is_onboarding_triggered": True, "actionable_items": "Review new portal registration.",
    ...
    if "cherry@yourwebaccountant.com" in sender_email:
        portal_keywords = ["project", "portal", "missed", "due", "appointment", "daily digest", "planning"]
        if any(k in subject.lower() for k in portal_keywords):
            has_activity = "no activity" not in snippet.lower() and "completed" not in snippet.lower()
            ...
            return {
                ...
                "actionable_items": "Check portal for updates." if has_activity else "None",
    ...
    # 💎 Cherry's "Golden Senders" (Tier 1 - STAY IN INBOX)
    tier_1_overrides = [
    ...
    tier_override = None
    if any(x in sender_email for x in tier_1_overrides) or any(x in sender_domain for x in tier_1_overrides):
        logger.info(f"🌟 TIER 1 OVERRIDE: {sender_email} — forcing Tier 1 (No Archive).")
        tier_override = 1
    elif sender_email in whitelist_emails:
        logger.info(f"🛡️ WHITELIST HIT (Email): {sender_email} — forcing Tier 1.")
        tier_override = 1
    elif sender_domain in whitelist_domains and sender_domain not in ['gmail.com', 'outlook.com', 'hotmail.com', 'yahoo.com', 'icloud.com']:
    ...
    if any(x in sender_email for x in tier_2_overrides) or any(x in sender_domain for x in tier_2_overrides):
    ...
    tier_3_overrides = [
        "superloop.com", "bigbosscleaning.com.au", "whapi.cloud", "telstra.com", "nab.com.au",
        "anz.com", "appsumo.com", "pinch.com.au", "suitedash.com", "linkedin.com", "simplefund360"
    ]
    if any(x in sender_email for x in tier_3_overrides) or any(x in sender_domain for x in tier_3_overrides):
        logger.info(f"TIER 3 SUPPLIER: {sender_email} — Log to Supplier_Expenses, stay in Inbox.")
        tier_override = 3

    # 📱 Business 1300 / Suitedash Digests handled below...

    # 📱 Business 1300 Call Centre Messages
    if "messages@business1300.com.au" in sender_email and "call centre message" in subject:
    ...
    if "cherry@yourwebaccountant.com" in sender_email and ("registration" in subject.lower() or "new user" in subject.lower() or "welcome" in subject.lower()):
    ...
    if "planning - daily digest" in subject.lower() or "planning daily digest" in subject.lower():
        has_activity = "no activity" not in snippet.lower() and "no new activity" not in snippet.lower()
        ...
        return {
            ...
            "is_portal_digest": True,
            "has_activity": has_activity,
            "actionable_items": "Reminder: Check Suitedash activity." if has_activity else "None",
            "needs_hitl_check": has_activity
        }
    
    if "document has been signed" in subject:
        logger.info("✍️ DOC SIGNED: Forcing Tier 2 (Log to Portal_Updates, Keep in Inbox).")
        return {
            "tier": 2,
    ...
    spam_keywords = ["unsubscribe", "no-reply", "promotion", "marketing", "offering 50%", "lottery"]
    if any(k in snippet for k in spam_keywords) or any(k in subject for k in spam_keywords):
        return {"tier": 5, "reason": "Automated rule: Spam/Marketing keywords detected.", "actionable_items": "None", "is_expense": False}

    # 2. Automated Portal/System Updates (Tier 4)
    portal_keywords = ["notification", "digest", "daily update", "system alert", "password reset", "verification code"]
    if any(k in subject for k in portal_keywords):
        return {"tier": 4, "reason": "Automated rule: System notification detected.", "is_portal_update": True, "actionable_items": "None"}

    # 3. Simple Politeness (Tier 4)
    if len(snippet.split()) < 5 and any(k in snippet for k in ["thanks", "thank you", "ok", "got it"]):
        return {"tier": 4, "reason": "Automated rule: Short polite reply.", "actionable_items": "None"}
...
def triage_email(email_content, client_context=None):
    ...
    try:
        ...
        context_str = f"EXISTING CLIENT INFO: {client_context}" if client_context else "NEW CONTACT (Not in Master List)."
        ...
        Tier 1: Active Clients / Whitelist. (Coiera, Angelo, David, John Watts). Action: STAY IN INBOX. DO NOT auto-draft unless Cherry explicitly asks.
        ...
        response = client.models.generate_content(
            model='gemini-2.0-flash',  # Ultra-fast triage
            contents=prompt,
            config={
        ...
        fallback_prompt = email_content.get('subject', '') + ' ' + email_content.get('snippet', '')
...
def draft_response(email_content, tier, historical_context="", client_info=None):
    """Generates a professional draft response using Tier logic and client background."""
    try:
        client = get_gemini_client()
        if not client:
        ...
        if tier in ['1', 1]:
            try:
                ...
                relevant_corrections = db.get_learned_patterns(sender_email=email_sender, subject=email_subject, limit=5)
                if relevant_corrections:
                    corrections_context = "\n\nCHERRY'S PREVIOUS CORRECTIONS FOR SIMILAR EMAILS (learn her preferences):\n"
                    for c in relevant_corrections:
                        corrections_context += f"- Feedback: {c.get('feedback', 'N/A')}\n"
                        corrections_context += f"  Original draft: {c.get('original_draft', '')[:150]}...\n"
                        corrections_context += f"  Corrected to: {c.get('corrected_draft', '')[:150]}...\n"
                else:
        ...
        if sender_instructions:
            ...
            if "builder tracker" in inst_text or "builder" in sender_info.get('category', '').lower():
                suggested_actions.append({"action": "add_to_sheet", "sheet": "Builder_Tracker", "label": "Add to Builder Tracker"})
            if "compare quotes" in inst_text:
                suggested_actions.append({"action": "compare_quotes", "label": "Compare Quotes"})
            if "cost-saving" in inst_text or "insight" in inst_text:
                suggested_actions.append({"action": "provide_insight", "label": "Provide Cost Insights"})
        
        prompt = f"""
        You are Cherry Dimapilis, founder of YourWebAccountant. Rewrite this email AS IF YOU wrote it.
        ...
        3. Use FIRST PERSON: "I" or "We" - NEVER "Cherry finds", NEVER "BackPocket Twin"  
        4. Maximum 4 sentences
        5. END with EXACTLY: "Talk soon, Cherry" or "Thanks, Cherry" (pick one based on tone)
        ...
        Rewrite Cherry's reply (use their first name if known):
        """
        ...
        return {
            ...
            "sender_instructions": sender_info.get('instructions', '') if sender_info else ''
        }
    except Exception as e:
...
def refine_draft(email_content, original_draft, feedback):
    ...
    try:
        ...
        # Load previous corrections to learn Cherry's preferences
        corrections_learn = ""
        try:
            import services.database as db
            ...
            if recent:
                ...
                for c in recent[:5]:
                    ...
                    corrections_learn += f"  Corrected: {c.get('corrected_draft', '')[:100]}...\n"
        except:
            pass

        prompt = f"""
        Refine this email draft based on feedback. Fix it to sound like Cherry wrote it.
        
        MANDATORY RULES:
        - Use "I" or "We" - NEVER "BackPocket Twin" or "Cherry finds"
        - End with EXACTLY: "Talk soon, Cherry" or "Thanks, Cherry"
...
def batch_triage_emails(emails_list: list):
    ...
    try:
        client = get_gemini_client()
        if not client:
            return {e.get('id'): {"tier": 5, "reason": "Gemini not initialized"} for e in emails_list}
    ...
    - TIER 5 (SPAM): Anything else (Marketing, Promo, Unsubscribe). Action: Trash.
        ...
        response = client.models.generate_content(
            model='gemini-2.0-flash',  # Ultra-fast batch triage
            contents=prompt,
        ...
        logger.error(f"GEMINI BATCH FAIL: {e}. Falling back to individual Ollama triage...")
        
        fallback_results = {}
        for email in emails_list:
            msg_id = email.get('id')
            logger.info(f"💾 LAYER 2 FALLBACK: Triaging {msg_id} individually via Ollama...")
            ...
            if ollama_res:
                ...
                fallback_results[msg_id] = {"tier": 5, "reason": "Ollama connection/timeout error"}
