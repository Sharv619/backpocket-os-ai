...
async def startup_event():
    # Skip Google Sheets check (needs credentials)
    # Skip polling loop (needs Gmail credentials) - runs once on demand instead
    # Background tasks disabled for demo mode
    pass
...
async def test_triage(subject: str, snippet: str):
    ...
    draft = (
        await loop.run_in_executor(None, draft_response, email_content, tier)
        if tier == 1
        else "No draft needed (Tier 2+)"
    )
    return {"triage": triage, "ai_draft": draft}
...
async def api_approve(request: ApproveRequest):
    ...
    if not email_addr or email_addr == "unknown sender" or "@" not in email_addr:
        return {
            "status": "error",
            "message": f"Cannot send - invalid recipient: '{email_addr}'. Please check the sender address in the pending email.",
        }
...
async def api_add_client_from_email(request: ApproveRequest):
    ...
    # Log for debugging
    logger.info(
        f"🔍 Extracting client from Ref #{ref_id}, email body length: {len(raw_email_body)}"
    )
...
# ============================================================================
# DOCUMENTS API - Upload, Analyze, Manage Documents
# ============================================================================
...
async def get_documents():
    """Get all documents."""
    try:
        from services.document_vision import get_all_documents
...
async def get_document(doc_id: int):
    """Get single document."""
    try:
        from services.document_vision import get_document
...
@app.post("/api/documents/analyze/{doc_id}")
async def analyze_document(doc_id: int):
    """Analyze document with Moondream AI."""
    try:
        from services.document_vision import analyze_document
...
async def delete_document(doc_id: int):
    """Delete a document."""
    try:
        from services.document_vision import delete_document
