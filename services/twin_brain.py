...
def get_sender_instructions():
    ...
    try:
        ...
        cur.execute("PRAGMA table_info(sender_instructions)")
        columns = [row[1] for row in cur.fetchall()]

        # Build query based on available columns
        if "is_critical" in columns:
            cur.execute("""
        ...
        for row in rows:
            inst = {
                ...
                "category": row[2],
                "instructions": row[3],
                "is_critical": row[4] if "is_critical" in columns else False,
                "created_at": row[5] if "is_critical" in columns else row[4],
                "updated_at": row[6] if "is_critical" in columns else row[5],
            }
...
def build_twin_context(sender_email=None, category=None):
    ...
    if sender_email:
        filtered_sender_inst = [
            s
            for s in sender_instructions
            if s["sender"].lower() == sender_email.lower()
        ]
    else:
...
def get_conversation_summary():
    ...
    try:
        ...
        for row in reversed(rows):
            messages.append({"role": row[0], "message": row[1], "time": row[2]})
    except Exception:
        pass
    finally:
        conn.close()
