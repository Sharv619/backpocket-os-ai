...
def add_message(session_id: str, role: str, content: str):
    """Add a message to conversation memory (legacy — uses chat_messages)."""
    save_message_instant(session_id, role, content)
...
def get_conversation(session_id: str, limit: int = 10) -> List[Dict]:
    """Get recent conversation history (legacy — uses chat_messages)."""
    return get_conversation_messages(session_id, limit)
...
def get_memory_context() -> str:
    ...
    # Get recent conversations from chat_messages
    try:
        cursor.execute("""
